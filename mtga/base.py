...
class MTGReader(object):
    ...
    def __init__(self,
        ...
        cache_file_dir = f"data_type={data_type}::set_code={set_code}::limited_type={limited_type}"
        ...
        with gzip.open(self.file_path, 'rt') as file:
            ...
            print(f"Created reader with the following non-card columns:\n{', '.join(self.noncard_columns)}.")
        self._n_lines = None
    ...
    def set_card_meta(self, header):
        ...
        for column in header:
            ...
            if is_card:
                ...
                self.card_meta[card_position] = self.card_meta.get(card_position, []) + [card_name]
    ...
    def get_data(self, force_refresh=False):
        is_written = os.path.exists(self.cached_noncard_data) and os.path.exists(self.cached_card_data)
